...
#mergedTestData <- mergeActivitySubjectData("Test","./test/subject_test.txt","./test/y_test.txt","./test/X_test.txt" )

#...Get train data, eliminate unnecessary columns and combine it with Subject and Activity
#mergedTrainData <- mergeActivitySubjectData("Train","./train/subject_train.txt","./train/y_train.txt","./train/X_train.txt" )


#..now merge test and trainig data
#merge(mergedTestData, mergedTrainData, by.x=c("TestSubject","TestActivity"), by.y=c("TrainSubject","TrainActivity"))
